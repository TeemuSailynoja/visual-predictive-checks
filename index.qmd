...
The posterior inference is often performed using Monte Carlo methods [@strumbelj_past_2024], and posterior, posterior 
...
In posterior predictive checking, we compare the data distribution to the posterior predictive distribution. The predictive distribution is not expected to coincide exactly with the observed data. For example, the posterior predictive distribution of a Bayesian model typically exhibits heavier tails than the true underlying data-generating distribution. Consequently, effective visual comparisons should focus on whether the the predictive distribution is consistent with observed data rather than on exact agreement. If these distributions are not similar, the model is misspecified and we should consider improving the model.
...
The discussed methods and recomendations in this work have been implemented in `bayesplot` (@gabry_plotting_2022) and `ArviZ` (@kumar_arviz_2019) software packages to facilitate their use.


## Visual predictive checks for continuous data{#sec-continuous-predictive-distributions}

In this section, we consider visualizations for observations from a *continuous* or *almost everywhere continuous* distribution. We focus on three data visualizations; histograms, and KDE-based continuous density plots, as they are the two most commonly used visualizations for summarizing unidimensional distributions, and quantile dot plots [@kay_when_2016] as we see that they offer a useful alternative that we recommend in many cases. @tbl-pros-cons summarizes the main advantages and disadvantages of these three visualizations, and @fig-intro-triplet shows a side-by-side comparison of these visualizations.
Histograms and density plots are implemented in various commonly used software packages for data visualization and are the two most common choices for initial visual PPCs (@wickham_ggplot2_2016;@gabry_plotting_2022; @kumar_arviz_2019; @kay_ggdist_2023; @eabm_2025).
...
Overlaid predictive checks. KDE plots can easily be overlaid and the resulting visualization is straightforward to read. The histogram for the data is overlaid with means and central 90% quantiles of the histogram bins of the posterior draws, when using the binning from the histogram of the observation. For the quantile dot plot, the top dot of each stack of dots in each posterior draw is overlaid to the full quantile dot plot of the observation, visualizing the variation in the shape of the predictive distributions.

:::

Overlaying facilitates comparisons by plotting model predictions and observed data on the same axes, allowing direct visual assessment. However, it can introduce overlap that may obscure information for the viewer, as we demonstrated with quantile dot plots. Alternative strategies include juxtaposition, nested juxtaposition, and explicit encoding [@guo_2025].
...
One approach to deal with binary data is the use of binned calibration plots [@niculescu-mizil_predicting_2005], also called calibration curves or reliability diagrams [@degroot_comparison_1983]. In these visualizations, the binary observations are divided into a predetermined number of uniform bins, based on the event probabilities predicted by the model. Comparing the mean predicted probability to the observed event rate within each bin allows assessing the calibration, or reliability, of the model. Typically, binned calibration plots include a binomial confidence intervals of the observed event rates.

@fig-binomial-binned-calibration shows the binned calibration plots for the predictive distributions visualized previously through bar graphs. It is immediately clear that even though the probabilities predicted by the intercept-only model are calibrated, the model has no ability to discriminate between the cases where one is more or less likely to observe roaches. We can also see, that although the negative binomial model is better at discriminating the cases, it suffers from calibration issues and is especially under-confident at predicting cases where no roaches will be observed.
...
To compute the residuals we can use binning to transform discrete outcomes into relative event frequencies, enabling residual-like diagnostics in this setting. But as already discussed for binned calibration plots the ad-hoc choice of binning may result in vastly different plots, and thus we propose the use of PAV-adjusted CEPs, computed with regards to the predicted probabilities, in place of the discrete observations. @fig-residuals shows an example of a PAV-adjusted residual plot for the
roaches example. 
...
the data being visualized. In this section, we introduce a method for assessing the goodness of fit of these visualizations, enabling users to evaluate whether a given choice is appropriate for their data. The proposed goodness-of-fit method is general and extends beyond the context of predictive checking.
