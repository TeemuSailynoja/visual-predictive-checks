...
###### Demonstration
We provide guidance for visual predictive checks across continuous, discrete, and mixed data. This includes diagnostics for identifying discrete components, estimating bounds, and assessing goodness-of-fit for density plots. We also offer recommendations for visual checks tailored to binary, ordinal, categorical, and count data.

###### Conclusion
We offer recommendations and diagnostic tools to mitigate ad-hoc decision-making in visual predictive checks. The discussed methods have been implemented in `bayesplot` (@gabry_plotting_2022) and `ArviZ` (@kumar_arviz_2019) software packages to facilitate their use. These contributions aim to improve the robustness and interpretability of Bayesian model criticism practices.
:::
...
When comparing data and model predictions, one approach is to show visualization of data  and visualization of the draws from the predictive distribution overlaid in one plot. The predictive distribution is not expected to give an exact match, for example the posterior predictive distribution of a Bayesian model usually has thicker tails than the true underlying observation distribution. As a result, effective visual comparisons should assess whether the observed data are plausibly consistent with the predictive distribution, rather than whether the two match exactly. For continuous data, a common way to do this is to plot overlaid density estimates.

Typically, the most common choice for overlaid density plots is the KDE, since multiple curves are usually easy to distinguish when shown together. The bandwidth selection plays an important role in the comparison. On one hand, if the chosen bandwidth is narrow, the KDEs can show large variation even when repeatedly drawing from the same distribution. On the other hand, with a wide bandwidth, the KDE may be too smooth and hide important details. 
...
While binned and PAV-adjusted calibration plots are useful for assessing the overall predictive calibration of a model, practitioners are often also interested in model behavior conditional on individual predictors. As discussed for continuous data, residual plots are commonly used to assess deviations between observations and predictions. While residuals are straightforward to compute and visualize for continuous outcomes, they are more challenging for binary data. 

As with binned calibration plots, binning is therefore used to transform discrete outcomes into relative event frequencies, enabling residual-like diagnostics in this setting. Again, the ad-hoc choice of binning may result in vastly different plots, and thus we propose the use of PAV-adjusted CEPs, computed with regards to the predicted probabilities, in place of the
...
introduced in @sec-predictive-checks_overlay, and our version of the rootogram [@kleiber_visualizing_2016].
...
In @sec-counts-with-ecdf we show how, similarly to the cases in @sec-continuous-predictive-distributions, goodness-of-fit tests can be used to assess how well the KDE plot represents the data. In @sec-counts-with-rootogram we introduce our version of the rootogram, and demonstrate its use when the familiar KDE exhibits goodness-of-fit issues. The visual predictive checks in both @sec-counts-with-ecdf and @sec-counts-with-rootogram follow the roaches example introduced in @sec-visual-predictive-checks-for-binary-data. We focus on assessing the quality of the predictions of a generalized linear model with a negative binomial data model for the number of roaches caught in traps during the original experiment.
...
PIT-ECDFs and LOO-PIT-ECDFs, previously discussed for continuous data, can also be applied to count data. Because count data are discrete, PIT values require careful handling; one solution is to use randomized PIT values, as described by [@czado_2009]. PAVA-adjusted calibration plots, which we discussed in the context of binary, ordinal, and categorical data—where they are particularly useful due to the lack of alternative strategies—can also be applied to count data. Residuals are commonly used as well, and the previously mentioned hanging and suspended rootograms can serve as residual plots for count outcomes.
